...
def phi_i(pdb_file, xtc_file, pairs_indexes, r_initial, boundaries, \
    ...
    for chunk_trajectory in md.iterload(xtc_file, top=pdb_file, chunk=chunk):
        ...
        contacts_time = np.sum(num_below_threshold, axis=1) # (number of contact per timestep)
...
def main():
    ...
    first_lower, first_upper = return_ordered(float(sys.argv[5]), \
                                              float(sys.argv[6]))
    ...
    third_lower, third_upper = return_ordered(float(sys.argv[9]), \
                                              float(sys.argv[10]))
